...
- Different existing contents are a conflict, not evidence of corruption: **Replace**, **Replace All**, rename, or skip. Replacement requires explicit approval and recovery storage for the displaced destination. Atomic exchange is preferred; qualified local filesystems without it use the retained-backup path below without extra prompts. **Replace All** still checks each existing file and does not rewrite verified matches. Unreadable or changing files are not treated as mismatches.
...
- On qualified local filesystems without atomic exchange (including exFAT), **Replace** first durably retains the old destination with a no-replace rename, then publishes the prepared new entry with another no-replace rename. This is not an atomic swap: interruption can leave the destination name absent, with old and new data retained in recovery/source locations. A racing new destination is never overwritten. No-replace rename, owned recovery storage, and checked synchronization remain mandatory; other I/O errors do not enable a weaker overwrite.
