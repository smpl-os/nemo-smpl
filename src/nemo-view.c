                                          ...
                                          gpointer   callback_data)
{
     ...
     * selected file. */
    to_favorite = !nemo_file_get_is_favorite (NEMO_FILE (selection->data));
    ...
    if (selection_not_empty_in_menu_callback (view, selection)) {
        ...
        for (iter = selection; iter != NULL; iter = iter->next) {
            ...
            nemo_file_set_is_favorite (file, to_favorite);
        }
    }
...
reset_open_with_menu (NemoView *view, GList *selection, gboolean filter_default)
{
	...
	for (node = applications, index = 0; node != NULL; node = node->next, index++) {
		...
		if (submenu_visible) {
			...
			menu_path = (char *)NEMO_VIEW_MENU_PATH_APPLICATIONS_PLACEHOLDER;
			popup_path = (char *)NEMO_VIEW_POPUP_PATH_APPLICATIONS_PLACEHOLDER;
		}
